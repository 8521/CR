...
public class f001013_QCnewreportequipman extends Fragment implements View.OnClickListener {


    private int equipmanposition;
    ...
    private ArrayList<EquipPeopleHours_QC_Object> selectedequipman;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
        ...
        selectedequipman = currentreport.getSelectedEquipPeopleHours();

        int lightred = ContextCompat.getColor(getActivity(),R.color.selection_light_orange);

        View v = inflater.inflate(R.layout.f01013_qcnewreportmanequip,container,false);
    ...
    private class adapter_7 extends RecyclerView.Adapter<viewholder_7> implements Interface_Adapter_7 {
        ...
        public viewholder_7 onCreateViewHolder(ViewGroup parent, int viewType) {
            View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.rvitem_07_qcreport_equipman,
                    parent,false);
        ...
        public void onBindViewHolder(viewholder_7 holder, int position) {
            holder.equipment.setText(selectedequipman.get(position).getEquipment1());
            holder.people.setText(String.valueOf(selectedequipman.get(position).getNumberOfPeople1()));
            holder.hours.setText(String.valueOf(selectedequipman.get(position).getHours1()));
        }

        @Override
        public int getItemCount() {
            return selectedequipman.size();
        }
        @Override
        public boolean onItemMove(int fromPosition, int toPosition) {
            Collections.swap(selectedequipman,fromPosition,toPosition);
        ...
        public void onItemDismiss(final int position) {
            final EquipPeopleHours_QC_Object temptask = selectedequipman.get(position);
            Snackbar snackbar = Snackbar
                    .make(mrecyclerView_7, "ITEM REMOVED", Snackbar.LENGTH_LONG)
                    .setAction("UNDO", new View.OnClickListener() {
                        @Override
                        public void onClick(View view) {
                            selectedequipman.add(position,temptask);
                            notifyItemInserted(position);
                            mrecyclerView_7.scrollToPosition(position);
                        }
                    });
            snackbar.show();
            selectedequipman.remove(position);
    ...
    private class viewholder_7 extends RecyclerView.ViewHolder implements Interface_ViewHolder_7,View.OnClickListener{
        ...
        final TextView hours;

        private viewholder_7(View itemView) {
            super(itemView);
            equipment = itemView.findViewById(R.id.input1);
            people = itemView.findViewById(R.id.input2);
            hours = itemView.findViewById(R.id.input3);
        ...
        public void onClick(View v) {
            ...
            f001013_z2_qcnewreportequipmanedit_df dfgmt = new f001013_z2_qcnewreportequipmanedit_df();
            equipmanposition = getAdapterPosition();
            mprefedit = mpref.edit();
            mprefedit.putInt("equipmannumber",equipmanposition);
            mprefedit.apply();

            dfgmt.setOnDismissListener(new DialogInterface.OnDismissListener() {
                @Override
                public void onDismiss(DialogInterface dialog) {
                    madapter_7.notifyDataSetChanged();
                }
            });
            dfgmt.show(getFragmentManager(),"Dialog");
        }
    ...
    private void ExitMethod() {
        currentreport.setSelectedEquipPeopleHours(selectedequipman);
        if(reportnumber==-1){
    ...
    private void EditTaskDialogFragment() {
        currentreport.setSelectedEquipPeopleHours(selectedequipman);
        mprefedit= mpref.edit();
        mprefedit.putInt("equipmannumber",-1);
        mprefedit.apply();

        f001013_z2_qcnewreportequipmanedit_df dialog_choose = new f001013_z2_qcnewreportequipmanedit_df();
